...
function getUniversalFigureRulesText() {
    return [
        '【图形高精度重建协议】（数学/物理/化学/生物/地理通用）',
        '阶段1：先抽取后绘制（禁止跳步）',
        '1) 先提取全部图形元素，再开始SVG绘制；未完成清单不得下笔。',
        '2) 图元元素必须完整识别：点/线段/射线/曲线/圆弧/角/多边形/坐标轴/函数图像/箭头/仪器/结构轮廓等。',
        '3) 符号元素必须单独识别：字母、数字、角标、上下标、单位、希腊字母、向量箭头、正负号、倍率符号。',
        '阶段2：空间关系矩阵（逐条落实）',
        '4) 所有可判定关系都要明确：上下/左右/内外/中间/邻接/重合/相交/相切/接触/包含/平行/垂直/共线/同心/对称。',
        '5) 若题干写明“在…上/下/内/外/左/右/之间”，SVG中必须肉眼可见同样关系，不得语义正确但图像错误。',
        '阶段3：坐标与比例（保证可验证）',
        '6) 先确定锚点（圆心、交点、端点、受力点、器官中心、地理目标区），再放置其他元素，避免关系漂移。',
        '7) 比例必须反映题意（长短、高低、远近、角度大小）；标签不得越界、遮挡关键元素或与锚点错位。',
        '阶段4：一一映射与禁止项',
        '8) elements中的每个元素都要在SVG中唯一对应：禁止漏画、并项、改名、改符号、错位、越界。',
        '9) 禁止添加题干未出现的装饰图元、背景纹理、无关图例；仅绘制解题所需对象。',
        '10) 线条和文字统一白色，禁止黑色；推荐viewBox=320x180；必须输出完整<svg>...</svg>。',
        '阶段5：输出前强制自检',
        '11) 自检顺序：元素完整性 → 符号准确性 → 空间关系一致性 → 比例合理性 → 可读性；任一失败必须重绘。'
    ].join('\n');
}
...
function getFigureFieldGranularityText() {
    return [
        '【figure字段颗粒度模板（有图时强制）】',
        '1) figure.elements必须“每个元素一行”，格式：元素名|类型|符号文本|数量/状态|绝对位置|关键坐标|关联元素。',
        '2) figure.elements要同时覆盖图元元素与符号元素；符号不可省略（如A、B、θ、F1、Δx、+、-）。',
        '3) figure.spatial必须“每条关系一行”，格式：主体|关系|客体|补充说明(方向/距离/角度/接触点)。',
        '4) figure.spatial要覆盖题干全部可判定关系，禁止写“关系正确”“如图所示”这类空泛语句。',
        '5) 若无图：figure.type=none，figure.description/figure.spatial/figure.elements/figure.svg均返回空字符串。'
    ].join('\n');
}
...
function getFigureRenderOrderRulesText() {
    return [
        '【SVG绘制顺序（必须执行）】',
        'a) 先定viewBox与关键锚点，再画主体轮廓，再画内部结构，最后放置符号和文字标签。',
        'b) 点名靠近对应点；角符号贴近角内侧；长度/距离标注贴近目标线段中点附近。',
        'c) 力学箭头必须从受力点出发并指向受力方向；化学键型与键角要匹配；地理图例必须贴近对应区域。',
        'd) 任何标签若与图元重叠导致歧义，必须重新布局，确保一眼可判关系。'
    ].join('\n');
}

function getPhysicsFigureRuleText() {
    return '物理补充：若出现斜面+滑块/物块/小车，滑块只能与斜面上边界接触；接触点必须可见，绝不能陷入斜面内部。';
}
...
- figure.elements必须逐行列出，格式固定为：元素名|类型|符号文本|数量/状态|绝对位置|关键坐标|关联元素
- figure.spatial必须逐行列出，格式固定为：主体|关系|客体|补充说明(方向/距离/角度/接触点)
- 必须覆盖每一个图元元素与每一个符号元素（字母/数字/角标/单位/希腊字母/运算符）
...
- 每道题的figure.elements必须逐行列出，格式固定：元素名|类型|符号文本|数量/状态|绝对位置|关键坐标|关联元素
- 每道题的figure.spatial必须逐行列出，格式固定：主体|关系|客体|补充说明(方向/距离/角度/接触点)
- 必须覆盖每一个图元元素与每一个符号元素（字母/数字/角标/单位/希腊字母/运算符）
- 严禁空间关系错误：例如滑块陷入斜面、点被画到圆外、细胞器越出细胞膜、地理图例与目标区域错位
...
- figure.elements必须逐行列出，格式固定：元素名|类型|符号文本|数量/状态|绝对位置|关键坐标|关联元素
- figure.spatial必须逐行列出，格式固定：主体|关系|客体|补充说明(方向/距离/角度/接触点)
- 必须覆盖每一个图元元素与每一个符号元素（字母/数字/角标/单位/希腊字母/运算符）
- 严禁空间关系错误：例如滑块陷入斜面、点被画到圆外、细胞器越出细胞膜、地理图例与目标区域错位
- 若原题无图，figure.type返回none且figure.svg留空
